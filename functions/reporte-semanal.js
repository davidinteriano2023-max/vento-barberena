...
// mnt/VENTO BARBERENA/netlify/functions/reporte-semanal.js
var lib = require_reportes_common();
exports.handler = async function() {
  try {
    const { desde, hasta } = lib.rangoSemanaGT();
    ...
    const desdeTexto = new Date(desde.getTime() + 12 * 3600 * 1e3).toLocaleDateString("es-GT", optsDate);
    const hastaTexto = new Date(hasta.getTime() - 12 * 3600 * 1e3).toLocaleDateString("es-GT", optsDate);
    const html = '<h2 style="color:#E8142B">\u{1F4C8} Reporte semanal \u2014 Vento Barberena</h2><p style="color:#666">Per\xEDodo: ' + desdeTexto + " al " + hastaTexto + "</p><p><b>" + lib.escapeHtml(etiquetas.pedidoLabel) + ":</b> " + metricas.numCotizaciones + "<br><b>" + lib.escapeHtml(etiquetas.humanoLabel) + ":</b> " + metricas.numSolicitudesHumano + "</p><h3>Detalle de cotizaciones de la semana</h3>" + lib.formatearTablaCotizaciones(metricas.pedidos) + '<p style="color:#888;font-size:12px;margin-top:20px">Este reporte se puede configurar desde el panel interno de Vento Barberena.</p>';
    ...
    await lib.enviarCorreoResend({
      to: destinatarios,
      subject: "\u{1F4C8} Reporte semanal Vento Barberena \u2014 " + desdeTexto + " al " + hastaTexto,
      html
    });
